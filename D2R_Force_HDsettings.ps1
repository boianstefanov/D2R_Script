...
& "$PSScriptLauncher\D2R.exe" -username $username -password $cred -address $region

Start-Sleep 1

# Set High priority for the process in Windows
Get-WmiObject Win32_process -filter 'name = "D2R.exe"' | foreach-object { $_.SetPriority(128) }
